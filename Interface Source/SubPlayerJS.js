    ...
    parseSubtitle(updateRate){
        ...
        setInterval(function(){
            if(!subPlayerVideo.paused){
                ...
                var startRequest = Math.round(totalAVBytesDecoded ) ;
                var endRequest = Math.round(totalAVBytesDecoded + bitRate) ;
                packsSize = endRequest - startRequest;

                if(isNaN(startRequest)){
                    startRequest = 0;
                }
 ...
 */
    ...
    static onSeekBarClick(videoid) {
        ...
        SubPlayerJS.getVideo(videoid).load();
        SubPlayerJS.getVideo(videoid).play();
        console.log(currentPosition);
        return false;
    }
